...
export const reserveTable = async (req, res, next) => {
  try {
    // Validate input
    const { error, value } = tableValidator.validateReservation.validate(req.body);
    if (error) {
      return next(new appError(error.message, 400));
    }

    const { tableId, duration, orderId } = value;
    ...
    releaseTime.setMinutes(releaseTime.getMinutes() + duration);
